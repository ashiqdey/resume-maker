      ...
      <Page size="A4" {...props} style={s.page}>
        ...
        <View style={getClass("row")}>
          ...
          <View style={getClass("col8 pr15")}>
            <View>
              ...
              <Text style={getClass("bio mt10")}>{bio}</Text>
            </View>
          </View>
          {/* END of left block */}
        ...
        {config.experience && (
          <View style={getClass("mt50")}>
            ...
            {experience.map((item, i) => (<ExpItems
              key={`exp-${i}`}
              s={s}
              getClass={getClass}
              title={item.company}
        ...
        {
          config.skills && <View>
            <Text style={getClass("h3")}>Profesional Skills</Text>
            <ThreeColumn data={skills} getClass={getClass} />
        ...
        {
          config.hobbies && <View wrap={false}>
            <Text style={getClass("h3")}>Hobbies</Text>
            {hobbies.map((e, i) => (
              <Item key={i} getClass={getClass}>{e}</Item>
            ))}
            <Divider style={s.hr} />
          </View>
          // END of hobbies
        ...
        {/* {
          config.languages && <View wrap={false}>
            <Text style={getClass("h3")}>Languages</Text>
            {languages.map((e, i) => (
              <Item key={i} getClass={getClass}>{e.language}</Item>
            ))}
            <Divider style={s.hr} />
          </View>
          // END of languages
...
const ThreeColumn = ({ data, getClass }) => {
  if (!data || data?.length === 0) {
    return null;
  }

  let perRow = Math.ceil(data.length / 3);

  const columns = [
    [...data.slice(0, perRow)],
    [...data.slice(perRow, perRow * 2)],
    [...data.slice(perRow * 2)],
  ];
  ...
  return (
    <View style={getClass("row")}>
      {
        columns.map((column, i) => <View key={i} style={getClass("col4 pr15")}>
          {
            column.map((e, i) => (
